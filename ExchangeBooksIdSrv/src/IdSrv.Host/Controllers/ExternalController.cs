...
namespace IdSrv.Host.Controllers
{
    ...
    {
        private readonly IApplicationUserStore<ApplicationUser> _applicationUserStore;
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            await HttpContext.SignInAsync(user.Email, user.UserName, provider, localSignInProps, additionalLocalClaims.ToArray());
            ...
            await _events.RaiseAsync(new UserLoginSuccessEvent(provider, providerUserId, user.Email, user.UserName, true, context?.ClientId));
        ...
        private async Task<(ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims)> FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user = await _applicationUserStore.FindByExternalProvider(provider, providerUserId);
        ...
        {
            var user = await _applicationUserStore.AutoProvisionUser(provider, providerUserId, claims.ToList());
            return user;
        }
