...
namespace AuthApi
{
    public class ClaimsAccessor : IClaimsAccessor
    {
        ...
        public string AccessToken => _httpContextAccessor.HttpContext.User.Claims.FirstOrDefault(c => c.Type == Constants.AccessToken)?.Value;

        public string Idp    => _httpContextAccessor.HttpContext.User.Claims.FirstOrDefault(c => c.Type == Constants.Idp)?.Value;
    }
}