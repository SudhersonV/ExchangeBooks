...
namespace IdSrv.Infra.CustomTokenProvider
{

    public abstract class CustomTotpSecurityStampBasedTokenProvider<TUser> : ICosmosUserTwoFactorTokenProvider<TUser>
            where TUser : class
    {
        /// <summary>
        ...
        /// <param name="manager">The <see cref="CosmosUserManager{TUser}"/> that can be used to retrieve user properties.</param>
        ...
        /// </remarks>
        public virtual async Task<string> GenerateAsync(string purpose, CosmosUserManager<TUser> manager, TUser user)
        {
            if (manager == null)
            {
            ...
            return CustomRfc6238AuthenticationService.GenerateCode(token, modifier).ToString("D4", CultureInfo.InvariantCulture);
        }
        ...
        /// </returns>
        public virtual async Task<bool> ValidateAsync(string purpose, string token, CosmosUserManager<TUser> manager, TUser user)
        {
            if (manager == null)
            {
                throw new ArgumentNullException(nameof(manager));
            ...
            var modifier = await GetUserModifierAsync(purpose, manager, user);
            return securityToken != null && CustomRfc6238AuthenticationService.ValidateCode(securityToken, code, modifier);
        }
        ...
        /// <paramref name="user"/> and <paramref name="purpose"/>.
        /// </returns>
        public virtual async Task<string> GetUserModifierAsync(string purpose, CosmosUserManager<TUser> manager, TUser user)
        {
            if (manager == null)
            {
                throw new ArgumentNullException(nameof(manager));
        ...
        /// </returns>
        public abstract Task<bool> CanGenerateTwoFactorTokenAsync(CosmosUserManager<TUser> manager, TUser user);
    }
}