...
namespace IdSrv.Infra.CustomTokenProvider
{
    ...
    public class CustomEmailTokenProvider<TUser> : CustomTotpSecurityStampBasedTokenProvider<TUser>
        where TUser : class
    {
        /// <summary>
        /// Checks if a two factor authentication token can be generated for the specified <paramref name="user"/>.
        /// </summary>
        /// <param name="manager">The <see cref="CosmosUserManager{TUser}"/> to retrieve the <paramref name="user"/> from.</param>
        ...
        public override async Task<bool> CanGenerateTwoFactorTokenAsync(CosmosUserManager<TUser> manager, TUser user)
        {
        ...
        public override async Task<string> GetUserModifierAsync(string purpose, CosmosUserManager<TUser> manager,
            TUser user)
        {
            var email = await manager.GetEmailAsync(user);
